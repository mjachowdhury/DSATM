...
public class MyStaticList{
	...
	public int getElement(int index) {
		...
		if ((index >= 0) && (index < numItems))
			...
			// Rule 2. Invalid index due to the current length of MyStaticList:
			// numItems <= index < maxItems
		...
		case 2:
			res = 0;
			System.out.println("ERROR: MyStaticList contains " + this.length()
					+ " elements, it is not possible to get an item at index " + index);
			break;
	...
	public void addElement(int index, int item) {
		...
		if ((index >= 0) && (index <= numItems)) {
			// Rule 1. Valid index and free room: (0 <= index <= numItems) and
			// (numItems < maxItems)
			if (numItems < maxItems)
				scenario = 1;
			// Rule 2. Valid index and full MyStaticList: (0 <= index <=
			// numItems) and (numItems < maxItems)
			else
				scenario = 2;
		} else {
			// Rule 3. Invalid index due to the current length of MyStaticList:
			// numItems < index < maxItems
			if ((numItems < index) && (index < maxItems))
				scenario = 3;
		...
		// (numItems < maxItems)
		case 1:
			// 1.1. We traverse all existing items from numItems-1 to index,
		...
		// Rule 2. Valid index and full MyStaticList: (0 <= index <= numItems)
		// and (numItems < maxItems)
		case 2:
			System.out.println("ERROR: MyList is already full");
			break;

		// Rule 3. Valid index but MyStaticList does not contain enough elements
		// so as to add to it: numItems < index < maxItems
		case 3:
			System.out.println("ERROR: MyList contains " + this.length()
					+ " elements, it is not possible to add an item at index " + index);
			break;
	...
	public void removeElement(int index) {
		...
		// Rule 1. Valid index: 0 <= index < numItems
		case 1:
			// 1.1. We traverse all existing items from index to 0, shifting
		...
		case 2:
			System.out.println("ERROR: MyStaticList contains " + this.length()
					+ " elements, it is not possible to remove an item from index " + index);
			break;
